(function (global) {
  global.preprocessTrainPathWithTime = function (graph, stations, train) {
    ...
    for (var i = 0; i < train.stops.length - 1; i++) {
      ...
      fromStop.ratioList = global.cumulativeSum(
  ...
  global.preprocessActiveTrainsPerTime = function (
    minUnixSeconds,
    maxUnixSeconds,
    trips,
    graph
    ...
    for (
      ...
      const active = trips.filter(
        global.isActiveObserved.bind(this, unixSeconds)
      );

      active
        .map(global.setTrainsPositions.bind(this, unixSeconds, graph))
