import { isActiveScheduled, isActiveObserved } from "./common.js";
import { subsectionsMaxCachedElements } from "./constant.js";
...
Graph.prototype.addPathToCache = function (source, target, path) {
...
Graph.prototype.searchPathInCache = function (source, target) {
...
Graph.prototype.shortestPath = function (source, target) {
  ...
  while (tail < queue.length) {
    ...
    if (!neighbors) {
      ...
      if (
        !state.errors.stopNoNeighboor.find(function (d) {
          return d === u;
        })
      )
...
Graph.prototype.isEdge = function (source, target) {
  return _.contains(this.neighbors[source], target);
};

// My custom sections
export function SectionManager(sections) {
  this.sections = sections;
}

SectionManager.prototype.refreshAtTime = function (
  unixSeconds,
  positionedTrains,
  lastTime
) {
...
SectionManager.prototype.addTrainToSubsection = function (
  from,
  to,
  train,
  type,
  ...
  const answered = this.sections.find(function (section) {
    ...
    const dir0SubSection = section.subsections.find(function (subsection) {
      return from === subsection.from && to === subsection.to;
    });
    // Find if on subsections dir1
    const dir1SubSection = section.subsections.find(function (subsection) {
      return to === subsection.from && from === subsection.to;
    });
    // It can only be one
    if (dir0SubSection && dir1SubSection) {
      console.log(
        "Error trying to assign train to subsection: for given section, two matching subsections"
      );
      return false;
    }
    // If none stop
    ...
    const alreadyCachedTrain = cachedTrainsContainer.find(function (cached) {
      return cached.train.trip === train.trip;
    });
    ...
    if (cachedTrainsContainer.length > subsectionsMaxCachedElements) {
      matchingSubsection.atTime[type][cachedDir] = cachedTrainsContainer.slice(
        cachedTrainsContainer.length - subsectionsMaxCachedElements
      );
    }
    return true;
  });
...
export function graphPreprocessing(graph, sections) {
  // Assign sections and subsection to stations
  sections.forEach(function (section) {
    // for each section
    section.points.forEach(function (station) {
      if (!station.linkedSections.includes(section)) {
        station.linkedSections.push(section);
      }
    });
    section.subsections.forEach(function (subsection) {
      // for each subsection
      ...
      if (!toStation.linkedSubSections.includes(subsection)) {
        toStation.linkedSubSections.push(subsection);
      }
    });
  });
